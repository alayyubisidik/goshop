...
@section('contents')
    <x-breadcrumb :items="[['label' => 'Home', 'url' => '/'], ['label' => $product->name]]" />
                        ...
                        <div class="col-md-6 col-lg-7 col-sm-12 col-xs-12">
                            <div class="detail-info pr-30 pl-30">
                                    ...
                                    <div class="product-price primary-color float-left">
                                        @if ($product->primaryVariant)
                                            @if ($product->primaryVariant->special_price > 0)
                                                <span
                                                    class="current-price text-brand">Rp.{{ $product->primaryVariant->special_price }}</span>
                                                <span
                                                    class="old-price font-md ml-15">Rp.{{ $product->primaryVariant->price }}</span>
                                            @else
                                                <span
                                                    class="current-price text-brand">Rp.{{ $product->primaryVariant->price }}</span>
                                            @endif
                                        @else
                                            @if ($product->special_price > 0)
                                                <span class="current-price text-brand">
                                                    Rp.{{ $product->special_price }}
                                                </span>
                                ...
                                @foreach ($product->attributeWithValues as $attribute)
                                    <div class="attr-detail attr-size " style="margin-bottom: 30px">
                                ...
                                <div class="detail-extralink mb-50">
                                    ...
                                    <div class="product-extra-link2">
                                        <button type="submit" data-id="{{ $product->id }}"
                                            class="button button-add-to-cart add_to_cart " data-variant=""
                                            data-modal="false" data-id="{{ $product->id }}"><i
                                    ...
                                    <ul class="float-start">
                                        ...
                                        <li class="mb-5">Tags:
                                            @foreach ($product->tags as $tag)
                                                <a href="#"
                                                    rel="tag">{{ $tag->name }}</a>{{ $loop->last ? '' : ', ' }}
                                            @endforeach
                                        </li>
                                        <li>Stock:
                                            <span class="in-stock text-brand ml-5"><span class="stock-qty">
                                    ...
                                    <ul class="contact-infor mb-50">
                                        @if ($product->store->address)
                                            <li><img src="{{ asset('assets/frontend/dist/imgs/theme/icons/icon-location.svg') }}"
                                                    alt="" /><strong>Address:</strong><span>{{ $product->store->address }}</span>
                                            </li>
                                        @endif
                                        @if ($product->store->phone)
                                            <li><img src="{{ asset('assets/frontend/dist/imgs/theme/icons/icon-contact.svg') }}"
                                                    alt="" /><strong>Phone:</strong><span>{{ $product->store->phone }}</span>
                                            </li>
                                        @endif
                                        @if ($product->store->email)
                                            <li><img src="{{ asset('assets/frontend/dist/imgs/theme/icons/icon-contact.svg') }}"
                                                    alt="" /><strong>Email:</strong><span>{{ $product->store->email }}</span>
                                            </li>
                                        @endif
                                    </ul>
                                                                ...
                                                                <div class="desc">
                                                                    <div class="d-flex justify-content-between mb-10">
                                                                        <div class="d-flex align-items-center">
                                                                            <span
                                                                                class="font-xs text-muted">{{ date('F d Y', strtotime($review->created_at)) }}</span>
                                                                        </div>
                                                                        <div class="product-rate d-inline-block">
                        ...
                        <div class="col-12">
                            <div class="row related-products">
                                @foreach ($relatedProducts as $item)
                                    <x-product-card :product="$item" class="col-6 col-lg-4 col-xl-3 col-xxl-2" />
                                @endforeach
                            </div>
                        </div>
                    </div>
...
@push('scripts')
    <script>
        // alert("Product ID: {{ $product->id }} - Name: {{ $product->name }}");
        $(function() {
